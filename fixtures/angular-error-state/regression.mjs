...
const targetUrl = process.env.FORMPROOF_TARGET_URL ?? "http://127.0.0.1:4187";
...
try {
  ...
  await page.waitForFunction(
    (expected) => document.querySelector('[role="status"]')?.textContent?.trim() === expected,
    "Subscription saved for ada@example.com."
  );
  ...
  if (browserErrors.length > 0) {
    throw new Error(`Angular error-state fixture browser errors: ${browserErrors.join(" | ")}`);
  }

  console.log("Angular error-state regression: accessible error exposure and correction behavior preserved.");
} finally {
  await browser.close();
}
