# Static error-state repair evidence

This package is the tracked, reviewable record of FormProof's first dynamic-state/error benchmark repair. The Static HTML fixture begins with a visible invalid email state whose `aria-errormessage` target is not exposed through an accepted announcement technique. Its browser regression requires the invalid state and error reference to remain intact, then verifies that correcting and submitting the address clears the error and announces success.
...
Repository commit `22b43d21fab5acc587cb94a4bf495ae5bcc981ad` is the clean state before this Static error-state fixture and experiment were added. After checking out the completed milestone, reverse-apply `repair.diff` to reconstruct the exact error-announcement barrier, then use the Static error-state commands in the root README to repeat the experiment in a disposable worktree.
